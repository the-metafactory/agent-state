...
|----------|---------|--------|
| `ScaffoldFolders` | `arc install` / first run | Creates the four-folder layout, runs `migrations/0001_initial.sql` |
| `EnqueueWorkItem` | Host `onMessageAccepted` hook | Inserts a row into `work_items` with `status=pending` |
| `ClaimWorkItem` | Worker pulls next item | Atomically transitions `pending → claimed` |
| `ResolveWorkItem` | Work completes | Transitions `claimed → done` or `failed`; appends terminal event |
| `AppendEvent` | Any observable transition | Inserts an immutable row into `events` |
| `ReplayPending` | Host `onStart` hook | Walks `work_items` where `status IN (pending, claimed)`, re-emits to the worker |
